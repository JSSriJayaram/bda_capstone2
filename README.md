...
Execute the full script inside the container (the SQL file is mounted at `/tmp/taxi_analytics.sql`):

```bash
docker exec taxi-hive-server bash -c "export HADOOP_CLIENT_OPTS='-Xmx4g' && hive --hiveconf hive.execution.engine=mr -f /tmp/taxi_analytics.sql"
```
